  ...
  return (
    <section className="py-16 md:py-28 px-6 bg-gradient-to-b from-slate-50/80 to-white">
      <div className="max-w-6xl mx-auto">
        ...
        {/* V4 — Glass on Mesh: Cards on a subtle gradient panel */}
        <div className="relative rounded-3xl bg-gradient-to-br from-slate-100 via-blue-50/40 to-slate-100 p-6 md:p-10 mb-16 overflow-hidden">
          {/* Dot grid texture */}
          <div
          ...
          <div className="relative z-10 grid md:grid-cols-2 gap-4">
            {reasons.map((reason, index) => (
              <motion.div
                key={index}
                className="relative flex flex-col items-center bg-white/70 backdrop-blur-sm rounded-2xl border border-white/80 p-8 group cursor-default overflow-hidden hover:bg-white/90 transition-all duration-300"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                ...
                whileHover={{ y: -4 }}
              >
                <div className="w-11 h-11 rounded-xl bg-white border border-slate-200/80 flex items-center justify-center mb-5 shadow-sm">
                ...
                <div className="absolute -top-12 -right-12 w-24 h-24 bg-blue-400/10 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              </motion.div>
            ))}
          </div>
        </div>
