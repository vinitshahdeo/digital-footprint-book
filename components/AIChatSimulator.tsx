...
const RESPONSES: Record<Tab, string> = {
  chatgpt: `Vinit Shahdeo is a software engineer and open-source advocate based in India, currently working as an Engineering Lead. He is recognized as a GitHub Star — a distinction awarded by GitHub to developers who have made significant contributions to the open-source community.\n\nWith over 10,000 stars on his GitHub projects, Vinit is known for building developer tools and mentoring at Google Summer of Code (GSoC). He is also the author of "Digital Footprint For Software Engineers," a book that helps software engineers build an authentic and discoverable online presence.`,
...
const TAB_CONFIG: Record<Tab, TabConfig> = {
  chatgpt: {
    ...
    tabIcon: (active, dark) => (
      <ChatGPTIcon size={12} fill={active ? (dark ? '#e8eaed' : '#334155') : '#9aa0a6'} />
    ),
  ...
  claude: {
    ...
    tabIcon: (active, dark) => (
      <ClaudeIcon size={12} fill={active ? (dark ? '#e8eaed' : '#334155') : '#9aa0a6'} />
    ),
  ...
  perplexity: {
    ...
    tabIcon: (active, dark) => (
      <PerplexityIcon size={12} fill={active ? (dark ? '#e8eaed' : '#334155') : '#9aa0a6'} />
    ),
...
export default function AIChatSimulator() {
  ...
  const chatgpt = useAIChatAnimation(
    RESPONSES.chatgpt,
    useCallback(() => advanceTo('gemini'), [advanceTo])
  )
  const gemini = useAIChatAnimation(
    RESPONSES.gemini,
    useCallback(() => advanceTo('claude'), [advanceTo])
  )
  const claude = useAIChatAnimation(
    RESPONSES.claude,
    useCallback(() => advanceTo('perplexity'), [advanceTo])
  )
  ...
  return (
    ...
    >
      <div
        className={`rounded-xl border shadow-xl overflow-hidden transition-colors duration-300 ${isDark ? 'border-slate-700/40 bg-[#212121] shadow-black/20' : 'border-slate-200 bg-white shadow-slate-200/50'}`}
      >
        {/* Browser Tab Bar */}
        <div
          className={`border-b px-4 py-2 flex items-center gap-3 transition-colors duration-300 ${isDark ? 'bg-[#292a2d] border-[#3c4043]' : 'bg-[#dee1e6] border-[#c4c7cc]'}`}
        >
          ...
          {/* URL Bar */}
          <div
            className={`hidden sm:flex items-center gap-2 rounded-full px-3 py-1 text-xs max-w-[200px] ${isDark ? 'bg-[#35363a] border border-[#3c4043]/60' : 'bg-white border border-slate-200'}`}
          >
            <svg
            ...
            <span className={`truncate ${isDark ? 'text-[#9aa0a6]' : 'text-slate-500'}`}>
              {cfg.url}
            </span>
          </div>
        </div>
        ...
        >
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            {/* Top bar */}
            <div
              className={`flex items-center px-4 py-2.5 border-b transition-colors duration-300 ${isDark ? 'border-[#2f2f2f]' : 'border-slate-100'}`}
            >
              <div className="flex items-center gap-1.5">
                {(activeTab === 'gemini' || activeTab === 'perplexity') &&
                  TAB_CONFIG[activeTab].tabIcon(true, isDark)}
                <span
                  className={`text-sm font-medium ${isDark ? 'text-[#ececf1]' : 'text-slate-800'}`}
                >
                  {cfg.label}
                </span>
              </div>
              <span
                className={`text-[10px] ml-1.5 px-1.5 py-0.5 rounded ${isDark ? 'text-[#8e8ea0] bg-[#2f2f2f]' : 'text-slate-500 bg-slate-100'}`}
              >
                {cfg.model}
              </span>
            </div>
            ...
            >
              ...
              {(active.phase === 'thinking' ||
                ...
                >
                  <div className="flex justify-end">
                    <div
                      className={`rounded-2xl px-4 py-2.5 max-w-[80%] ${isDark ? 'bg-[#2f2f2f]' : 'bg-slate-100'}`}
                    >
                      <p className={`text-sm ${isDark ? 'text-[#ececf1]' : 'text-slate-800'}`}>
                        {QUESTION}
                      </p>
                    </div>
                  </div>
              ...
              <AnimatePresence>
                {active.phase === 'thinking' && (
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${cfg.accentBg}`}
                    >
                      {cfg.icon}
                    </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm leading-relaxed whitespace-pre-line ${isDark ? 'text-[#d1d5db]' : 'text-slate-700'}`}
                      >
            ...
            <div className="px-4 md:px-6 pb-3 pt-2">
              <div
                className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl border ${isDark ? 'border-[#3f3f3f] bg-[#2f2f2f]' : 'border-slate-200 bg-slate-50'}`}
              >
                <div className="flex-1 text-sm min-h-[20px] flex items-center">
                  {active.phase === 'idle' && (
                    <>
                      <span className={isDark ? 'text-[#8e8ea0]' : 'text-slate-400'}>
                        {cfg.placeholder}
                      </span>
                  ...
                  {active.phase === 'typing-question' && (
                    <>
                      <span className={isDark ? 'text-[#ececf1]' : 'text-slate-800'}>
                        {active.typedQuestion}
                      </span>
                      <TypewriterCursor dark={isDark} />
                    </>
                  )}
