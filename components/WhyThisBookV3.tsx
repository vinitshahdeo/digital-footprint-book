  ...
  return (
    <section className="py-16 md:py-28 px-6 bg-gradient-to-b from-slate-50/80 to-white">
      <div className="max-w-6xl mx-auto">
        ...
        {/* V3 — Left Border Accent: Gradient left edge with horizontal layout */}
        <div className="grid md:grid-cols-2 gap-4 mb-16">
          {reasons.map((reason, index) => (
            <motion.div
              key={index}
              className="relative bg-white rounded-xl pl-0 pr-6 py-6 group cursor-default overflow-hidden flex items-start gap-5"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              ...
              <div className="w-1 self-stretch rounded-full bg-gradient-to-b from-blue-400 to-blue-600 flex-shrink-0 group-hover:from-blue-500 group-hover:to-purple-500 transition-all duration-300" />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center group-hover:bg-blue-100 transition-colors duration-200">
                    <reason.icon className="w-4 h-4 text-blue-600" />
                  </div>
                  <h3 className="text-base font-semibold text-slate-900 group-hover:text-blue-600 transition-colors duration-200">
                    {reason.title}
                  </h3>
                </div>
                <p className="text-slate-500 text-sm leading-relaxed pl-12">{reason.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
