  ...
  return (
    <section className="py-16 md:py-28 px-6 bg-gradient-to-b from-slate-50/80 to-white">
      <div className="max-w-6xl mx-auto">
        ...
        {/* V5 — Compact Inline Chips: Horizontal pill-style layout */}
        <div className="flex flex-wrap justify-center gap-3 md:gap-4 mb-16">
          {reasons.map((reason, index) => (
            <motion.div
              key={index}
              className="inline-flex items-center gap-3 px-5 py-3.5 bg-white rounded-full border border-slate-200/60 group cursor-default hover:border-blue-200 hover:shadow-md transition-all duration-300"
            ...
            >
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center flex-shrink-0">
                <reason.icon className="w-3.5 h-3.5 text-white" />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-semibold text-slate-900 group-hover:text-blue-600 transition-colors duration-200 whitespace-nowrap">
                  {reason.title}
                </span>
                <span className="text-xs text-slate-400 leading-tight">{reason.description}</span>
              </div>
            </motion.div>
          ))}
        </div>
